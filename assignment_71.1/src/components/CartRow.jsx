...
function Cartproduct({ product, onRemove, onQuantityChange }) {
  return (
    <div className='w-full grid grid-cols-14 h-20 bg-white border-b border-gray-200 items-center'>
        <div className="col-span-1"><MdOutlineCancel onClick={() => onRemove(product.id)} className='mx-auto text-3xl text-gray-200 hover:text-red-500 cursor-pointer' /></div>
        ...
        <div className="col-start-11 col-end-13"><input onChange={(e) => onQuantityChange(product.id, e.target.value)} type="number" className='w-12 border border-gray-300 rounded-md p-1' value={product.quantity} /></div>
