...
function CartList({ products, cart }) {
  return (
    <div className='flex flex-col border-1 border-gray-200'>
                        ...
                        {products.length <= 0 ? <div className='flex flex-col justify-center items-center h-60 gap-4'>
                            <div className='text-center text-gray-500'>
                                Your cart is empty
                            </div>
                            <Link to='/' className='mt-4 bg-red-500 font-semibold text-white px-4 py-2 rounded'>Continue Shopping</Link>
                        </div> : products.map(product => {
                            const Item = { ...product, quantity: cart[product.id], subtotal: (cart[product.id] * product.price).toFixed(2) };
                            return <CartRow key={product.id} product={Item} />
                        })}
                        <div className='w-full grid grid-cols-14 h-14 bg-gray-50 items-center'>
                            <div className="col-span-6 pl-4 text-gray-700 font-semibold grid grid-cols-2">
                                ...
                                : "bg-red-500 hover:bg-red-300"} text-white px-4 py-1 rounded ml-2`}>Apply Coupon</button>
                            </div>
                            <button className={` ${products.length <= 0
                                ? "bg-red-200 cursor-not-allowed"
                                : "bg-red-500 hover:bg-red-300"} col-end-15 mr-4 text-white py-1 px-2 font-semibold col-span-3 rounded`}>Update Cart</button>
                        </div>
                    </div>
  )
}
